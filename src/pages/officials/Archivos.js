...
function Archivos() {
    ...
    const fraccionID = searchParams.get('fraccion');

    useEffect(() => {
        if (fraccionID) {
            localStorage.setItem('fraccionID', fraccionID);
        }
    }, [fraccionID]);

    useEffect(() => {
        const storedFraccionID = localStorage.getItem('fraccionID');
        const fetchData = async () => {
            if (storedFraccionID) {
                try {
                    // Obtener documentos
                    const responseDocumentos = await fetch(`http://localhost:8000/documento/fraccion/${storedFraccionID}`);
                    if (!responseDocumentos.ok) {
                        throw new Error('Error en la respuesta de la API');
                    }
                    const dataDocumentos = await responseDocumentos.json();
                    if (Array.isArray(dataDocumentos) && dataDocumentos.length > 0) {
                        setDatosDocumentos(dataDocumentos);
                    }
    
                    // Obtener fracciones
                    const responseFracciones = await fetch('http://localhost:8000/fraccion');
                    if (!responseFracciones.ok) {
                        throw new Error('Error en la respuesta de la API');
                    }
